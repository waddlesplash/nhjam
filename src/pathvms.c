...
static void dir_flags(const char* buf, int len, struct dirinf* i)
{
	...
	if (!buf || !len) {
		i->flags = DIR_EMPTY;
		i->dev.ptr = i->dir.ptr = 0;
		i->dev.len = i->dir.len = 0;
	...
	*file = 0;
	...
	if (DEBUG_SEARCH)
		printf("built %.*s + %.*s / %.*s suf %.*s mem %.*s -> %s\n",
			   f->f_root.len, f->f_root.ptr, f->f_dir.len, f->f_dir.ptr,
			   f->f_base.len, f->f_base.ptr, f->f_suffix.len, f->f_suffix.ptr,
			   f->f_member.len, f->f_member.ptr, ofile);
#endif
}
